...
또한 여기서 생기는 문제들을 정리하면 다음 3가지와 같다.
...
이번 변경은 단순히 `try-catch`를 없앤 것이 아니다.

각 조회 메서드는 YouTube API 요청을 구성하는 일에 집중하고, 공통 메서드는 반복되는 예외 변환을 담당하도록 책임을 분리한 것이다.

> `try-catch`를 없앤 것이 아니라, 반복되던 예외 처리 정책을 하나의 메서드로 모았다.
